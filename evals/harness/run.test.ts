...
describe("loadCases (golden set 무결성)", () => {
  ...
  it("qa 셋은 모두 기대 사실(must)을 가지며 전제반박 가드를 최소 1개 포함한다", () => {
    expect(qa.length).toBeGreaterThanOrEqual(4);
    expect(qa.every((c) => c.must.length > 0)).toBe(true);
    expect(qa.filter((c) => c.mustNot.length > 0).length).toBeGreaterThanOrEqual(1);
  });
  ...
  it("모든 케이스가 rule_source(근거)를 명시한다", () => {
    expect(cases.every((c) => c.ruleSource.trim().length > 0)).toBe(true);
  });
});
