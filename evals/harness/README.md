...
한 줄이 에이전트 행동을 바꾸므로, 하네스도 코드처럼 회귀한다. 두 축을 본다:

- **review 트랙** — 경량 리뷰어가 `CLAUDE.md`의 CRITICAL 룰 위반을 일관되게
  잡아내고, 정상 코드를 오탐하지 않는지.
- **qa 트랙** — `CLAUDE.md`가 코드베이스 질문(규약·예외처리·gotcha)에 정확히
  답할 만큼 충분한지, 그리고 응답자가 틀린 전제에 동조하지 않는지(환각 방지).
  gotcha 한 줄을 `CLAUDE.md`에서 지우면 해당 qa 케이스가 회귀로 잡힌다.
...
- **golden set**(`cases/*.md`): 입력 + 기대 라벨. `kind: review|qa`로 트랙을 가른다.
  ...
  - qa(4): 본문=질문, 라벨=`must`(반드시 담겨야 할 사실)·`must_not`(말하면 안 되는 오답).
    사실 검증 3 + 전제반박 가드 1.
- **리뷰어**(`reviewer.ts`): review 트랙 대상. `CLAUDE.md` CRITICAL 룰 요약(`rules.ts`)을
  시스템 프롬프트로 적용해 코드를 리뷰하는 경량 단일 호출(Sonnet).
  산문 리뷰를 출력한다.
- **응답자**(`responder.ts`): qa 트랙 대상. **라이브 `CLAUDE.md`**를 컨텍스트로 받아
  질문에 답하는 경량 단일 호출(Sonnet). 산문 답변을 출력한다.
...
evals/harness/
  ...
  rules.ts                CLAUDE.md CRITICAL 룰 요약 → 리뷰어 시스템 프롬프트
  reviewer.ts             review 트랙 대상 (Claude 호출)
  responder.ts            qa 트랙 대상 — 라이브 CLAUDE.md 컨텍스트 (Claude 호출)
  judge.ts                LLM-as-judge: judge(review) / judgeQa(qa)
...
```bash
# 전체 Eval (Claude 호출 — ANTHROPIC_API_KEY 필요, 레포 루트 .env 자동 로드)
npm run eval
...
분리돼 있다. 순수 함수(`lib/case.ts`, `lib/verdict.ts`)와 golden set 무결성
(review 위반4+정상1, qa must/가드 존재, id 유일성)만 `npm test`가 지킨다.
...
- **judge는 흔들린다.** `claude-sonnet-5`/`claude-opus-5`는 `temperature` 파라미터
  자체를 지원하지 않아(API가 거부한다) 모델 기본 샘플링에 맡긴다. LLM 채점은 완벽히
  결정적이지 않다는 뜻이다. 판정이 애매하면 사람이 최종 검토한다(judge 맹신 금지).
- **경량 리뷰어 ≠ 실제 `/review-code`.** 게이트 속도를 위해 단일 호출로 대리한다.
  산문 입출력이라 `reviewer.ts`만 headless `/review-code`로 교체하면 실제
  워크플로우를 eval할 수 있다(judge·게이트는 불변).
