...
// 각 케이스: subject(리뷰어/응답자, Sonnet)가 리뷰/답변 → judge(Opus)가 기대 대비 채점 → 집계.
// 하나라도 실패하면 exit 1 (CI/로컬 회귀 게이트). 키가 없으면 exit 2.
import { readdirSync, readFileSync } from "node:fs";
import path from "node:path";
import { pathToFileURL } from "node:url";

import { createClaudeJudge } from "./judge.ts";
import { type GoldenCase, parseCase } from "./lib/case.ts";
import { type CaseResult, formatSummary, summarize } from "./lib/verdict.ts";
import { createClaudeResponder } from "./responder.ts";
import { createClaudeReviewer } from "./reviewer.ts";

// npm test/npm run eval 모두 레포 루트에서 실행되므로 process.cwd() 기준으로 잡는다
// (import.meta.url 기반 상대 경로는 Vitest jsdom 환경에서 정적 import된 모듈에 한해
// 신뢰할 수 없게 평가되는 경우가 있어 피한다).
export const CASES_DIR = path.resolve(process.cwd(), "evals/harness/cases");
...
function loadEnvIfPresent(): void {
  try {
    ...
    // .env가 없으면(CI 등 환경변수 직접 주입) 조용히 넘어간다.
  }
}
