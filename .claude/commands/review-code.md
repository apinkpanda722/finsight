...
> 이 커맨드는 **Workflow 도구**를 사용하며 서브에이전트를 다수 소환한다(3 차원 리뷰 + 확정 후보당 3명 검증). 가벼운 체크리스트 리뷰는 `/review`를 써라.
...
2. **PR 연결 여부 확인**
   - `gh pr view --json number,headRefOid` 시도. 성공하면 `prNumber`, `headSha`를 얻는다. 실패(연결된 PR 없음)하면 `prNumber=null`로 둔다.
...
반환값: `{ confirmed: [{ severity, file, line, title, tldr, good, fix, dimension, votes }], stats: { total: { raw, confirmed, agents }, byDim, bySeverity } }`.
`confirmed`만 게시 대상이다(검증을 통과한 발견). 비어 있으면 판정은 **Approve**다. `stats.total.agents`는 이번 실행에서 실제로 소환된 에이전트 총수(리뷰 + verify)다.
...
`stats`로 집계해 아래 형식으로 만든다. Walkthrough는 diff를 보고 직접 2~3줄로 쓴다. **Critical/Major만** 나열하고 minor·nit은 인라인에만 남긴다. **제목 줄은 항상 고정 포맷**(`🤖 /review-code - 판정: {verdict}`)을 쓴다 — 다른 문구로 바꾸지 않는다.

```
## 🤖 /review-code - 판정: {Approve|Changes Requested|Blocked}
...
---
🤖 review-code workflow · {stats.total.agents}개 agent · {byDim 요약} 검출, verify 통과 {confirmed}/{raw}
```

`{byDim 요약}`은 `stats.byDim`의 각 차원을 `{dimension} {raw}` 형태로 만들어 ` + `로 연결한다(raw가 0인 차원은 제외). 전부 0이면 `검출 없음`이라고 쓴다. 예: `architecture 1` / `correctness 1 + security 2` / `검출 없음`.
...
- **`prNumber`가 없으면**: 콘솔에 **전체 요약(Layer 2) → 인라인(파일:라인 순, Layer 1)** 순서로 마크다운 출력한다. PR에 코멘트를 남기지 않는다.

PR에 코멘트를 게시하는 것은 다른 사람에게도 보이는 행동이므로, 게시 직전에 위 Layer 2 요약(및 발견 개수)을 보여주고 사용자 확인을 받은 뒤에만 실행한다.

차원별 발견 수와 검증 통과율은 Layer 2 footer에 이미 포함되므로 별도로 다시 요약하지 않는다.
