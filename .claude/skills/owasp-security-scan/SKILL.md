...
description: 레포지토리를 OWASP Top 10 2025 기준으로 하이브리드(결정론적 도구 + 카테고리별 병렬 LLM 서브에이전트) 보안 스캔하고, 단일 HTML 대시보드 + 구조화 JSON으로 결과를 낸다. "보안 스캔", "보안 점검", "security scan", "security audit", "OWASP", "owasp top 10", "취약점 스캔/점검", "이 레포 안전한지 봐줘", "보안 감사" 같은 요청에 트리거. 범용 OWASP 점검에 finsight 스택 휴리스틱(RLS·service_role 전용 RPC·Polar 웹훅 서명검증·getClaims 강제·로그 마스킹)을 추가로 적용한다. 결과는 항상 깔끔한 기술 대시보드 HTML(Inter+JetBrains Mono, light surface)이며, 발견은 adversarial 검증을 통과한 것만 critical/high로 싣는다.
user-invocable: true
---
...
- 인자가 git 범위(`main...HEAD` 등)면 변경분 한정 스캔(scope에 표기) — 다만 PR 단위 리뷰가 목적이면 `/review-code`를 먼저 권한다.

## 1. 결정론적 pre-scan
출력 경로는 레포에 `docs/`가 있으면 `docs/`, 없으면 `.claude/`. (사용자가 경로 지정 시 그 경로 우선. finsight는 `docs/`가 있으므로 기본은 `docs/`.)
...
먼저 다음을 읽어라:
- 루브릭: .claude/skills/owasp-security-scan/references/owasp-2025.md (담당 카테고리 섹션 + "finsight 보안 아키텍처 요약" 집중)
- 결정론 pre-scan 결과: <prescan.json 경로> (특히 leads의 담당 카테고리, supply_chain, secrets)

가드레일(finsight 규칙):
<CLAUDE.md/ARCHITECTURE/ADR 요약>

점검 범위: <scope> (전체 레포면 src/·supabase/migrations/·next.config·middleware 중심)
...
- A01/A08 담당이면 create_statement_upload/finalize_statement RPC가 service_role 전용이고 RPC 자체는 p_user_id를 검증하지 않는다는 사실을 반드시 고려해 호출부(route/서비스 계층)를 확인하라.
...
코드를 직접 Read해 교차검증하라: 실제로 악용 가능한가? 이미 다른 레이어(RLS 정책·getUser() 검증·webhook 서명검증 등)가 막고 있지 않은가? 존재하지 않는 라인/오해는 아닌가?
...
사용자가 링크로 공유하고 싶어하면, 생성한 HTML 파일을 `Artifact` 도구로 발행할 수도 있다(그 전에 `artifact-design` 스킬을 먼저 로드) — 기본 산출물은 로컬 파일이며, 이건 요청 시에만 추가로 한다.
...
## Common pitfalls
- **leads를 그대로 finding으로 베끼기** — leads는 단서다. 반드시 코드로 확인. 다수는 정상(예: 정당한 webhook 모듈에서의 service_role 사용).
...
- **finsight 휴리스틱 누락** — 범용 OWASP만 보면 RLS·service_role 전용 RPC·webhook 서명검증·로그 마스킹 같은 핵심 위반을 놓친다. 루브릭의 finsight 섹션을 반드시 적용.
- **secrets의 env_ref 오인** — `env_ref:true`는 placeholder/`process.env` 참조. 실제 유출 아님.
- **diff 리뷰와 혼동** — 이건 전체 레포 감사다. PR 단위는 `/review-code`.

## Files
- `references/owasp-2025.md` — OWASP Top 10 2025 × finsight 휴리스틱 루브릭(카테고리별 점검 기준 + severity 가이드 + finsight 보안 아키텍처 요약)
