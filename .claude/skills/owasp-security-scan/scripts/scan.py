...
SECRET_PATTERNS = [
    ...
    ("generic_assignment", re.compile(
        r"""(?ix)
        \b(api[_-]?key|secret|client[_-]?secret|password|passwd|access[_-]?token|auth[_-]?token|service_role[_-]?key)
        \s*[:=]\s*
        (['"])([^'"\s${}]{8,})\2
        """)),
]
...
# confirm or dismiss each against real semantics. Tuned to finsight's actual
# stack: Supabase (RLS + SECURITY DEFINER RPCs), Polar billing, Anthropic SDK.
LEAD_PATTERNS = {
    "A01": [  # Broken Access Control
        (re.compile(r"\.getSession\("), "authz를 getSession()으로 판단하면 위반 — getUser()/getClaims() 검증인지 확인"),
        (re.compile(r"\bservice_role\b|SERVICE_ROLE"), "service_role 클라이언트 사용처 — webhook/서비스 계층 한정인지, 일반 라우트에서 RLS를 우회하고 있지 않은지 확인"),
        (re.compile(r"p_user_id\s*:|userId\s*:"), "create_statement_upload/finalize_statement RPC에 넘기는 p_user_id/userId가 요청 body/param이 아니라 getUser()로 얻은 세션 사용자 id인지 확인"),
        (re.compile(r"(req|request)\.(body|headers|query|nextUrl)[^\n]{0,60}\b(user_?id|customerId|customerExternalId)\b", re.I), "요청 body/query의 user_id·customer id를 그대로 신뢰하면 IDOR — 서버 세션에서만 얻어야"),
    ],
    "A02": [  # Security Misconfiguration
        ...
        (re.compile(r"dangerouslyAllowBrowser\s*:\s*true"), "Anthropic/타 SDK를 브라우저에 노출 — 키 유출 위험"),
        (re.compile(r"rejectUnauthorized\s*:\s*false|NODE_TLS_REJECT_UNAUTHORIZED"), "TLS 검증 비활성화"),
    ],
    "A04": [  # Cryptographic Failures
        (re.compile(r"createHash\(\s*['\"](md5|sha1)['\"]"), "약한 해시(md5/sha1) — 무결성/식별 용도면 위반"),
        (re.compile(r"Math\.random\([^)]*\)[^\n]{0,40}(token|id|secret|nonce|otp|storage_?path)", re.I), "예측 가능한 난수를 보안 토큰/식별자에 사용 의심 — statement_id는 DB gen_random_uuid()가 생성해야 정상"),
    ],
    "A05": [  # Injection
        (re.compile(r"dangerouslySetInnerHTML"), "XSS — transaction description/file_name/error_message 같은 신뢰 불가 입력이 들어가는지 확인"),
        ...
        (re.compile(r"(SELECT|INSERT|UPDATE|DELETE|FROM|WHERE)[^\n;]{0,80}\$\{", re.I), "문자열 보간으로 만든 SQL 의심 — Supabase 쿼리 빌더/파라미터화 RPC 필요"),
    ],
    "A08": [  # Software or Data Integrity Failures
        (re.compile(r"validateEvent|constructEvent|verif(y|ied)Signature|webhookSecret|x-signature", re.I), "웹훅 서명검증 흔적(긍정 신호) — raw body 기준인지, 실패 시 거부하는지, polar_modified_at보다 새 이벤트일 때만 반영하는지 확인"),
        (re.compile(r"req\.body|request\.json\(\)"), "웹훅 라우트라면: 파싱 전 raw body 서명검증 + stale/역순 이벤트 방어가 있는지 확인"),
        (re.compile(r"\.insert\(|\.update\(|\.upsert\("), "accounts/uploaded_statements/transactions에 대한 직접 INSERT/UPDATE 의심 — create_statement_upload/finalize_statement RPC를 거치지 않으면 위반"),
    ],
    "A10": [  # Mishandling of Exceptional Conditions
        ...
        (re.compile(r"processing_lease|lease_expires"), "processing lease 관련 코드 — 예외 발생 시에도 lease가 해제/만료되어 좀비 lease로 재처리가 막히지 않는지 확인(긍정 신호)"),
    ],
}


# Lead-grep targets first-party source only. Skipping these avoids matching the
# CRITICAL-rule *descriptions* that live in skill prompts / doc text.
LEAD_SKIP_PREFIXES = (".claude/", "docs/")
