...
## 개발 프로세스
- CRITICAL: 새 기능 구현 시 반드시 테스트를 먼저 작성하고, 테스트가 통과하는 구현을 작성할 것 (TDD). `scripts/hooks/tdd-guard-claude.sh`(Claude Code hook 어댑터, `.claude/settings.json`이 등록)가 `src/**/*.ts(x)` 수정 전 동일 이름 테스트 파일 존재를 강제한다. 실제 예외 규칙은 `scripts/hooks/tdd-guard-core.sh`에 있다(Codex용 어댑터 `tdd-guard-codex.sh`와 공유). SDK 인스턴스화 전용 래퍼(`lib/{supabase,polar,anthropic}/*`)는 예외 — step 0에서 hook에 반영한다.
