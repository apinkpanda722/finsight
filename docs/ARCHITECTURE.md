...
│   ├── (dashboard)/
│   │   ├── layout.tsx
│   │   ├── dashboard/page.tsx        # 카테고리별 지출 요약 + 월별 추이 (전체 통합 뷰)
│   │   ├── uploads/page.tsx          # CSV/PDF 업로드 UI, statement 목록/삭제/재시도
...
- 사용자 소유 리소스를 참조하는 테이블 간에는 `(user_id, id)` unique + composite FK를 사용해 cross-user 연결을 DB가 거부한다(`transactions → uploaded_statements`)
...
- `accounts` 엔티티는 없다(ADR-011) — 모든 명세서/거래는 `user_id`로 사용자에 직접 귀속되고 계정 하나에 통합된다. 은행/카드명은 파싱된 표시용 라벨(`uploaded_statements.detected_label`)일 뿐 별도 테이블/FK가 아니다

## 데이터 흐름

**CSV/PDF 업로드 → 분석** (Vercel Function의 4.5MB 요청 본문 한도를 우회하기 위해 파일은 Function을 거치지 않는다):
```
1. 클라이언트: 확장자/5MB 사전 검증 → POST /api/statements/init-upload (메타데이터만)
2. 서버: create_statement_upload RPC (advisory lock 하 일 10회 업로드 한도 검사 + usage 기록 +
   statement(uploading) 생성을 한 트랜잭션으로) → signed upload URL 발급
3. 클라이언트: 원본 파일을 Supabase Storage에 직접 업로드 (서버를 거치지 않음)
4. 클라이언트: POST /api/statements/{id}/complete-upload
5. 서버: Storage 원본 다운로드 → 실제 크기·인코딩(UTF-8/CP949)·구조(CSV: RFC 4180 / PDF: 표 추출)·
   행 수(≤2,000) 검증 → 통과 시 status=pending, row_count 저장, 202 응답, after()로 백그라운드 처리 예약
6. 백그라운드: processing lease CAS 획득 → 결정론적으로 전체를 행 단위 파싱
   → Claude로 헤더+샘플(≤20행) 기반 ColumnMappingSchema + 은행/카드명(detected_label) 추론 (1회, 같은 호출)
...
8. 클라이언트: 2초 간격 최대 150회 폴링으로 상태 확인 → 대시보드 갱신
```
